...
# input data
ring_data = pd.read_csv('ring.csv', sep=',')

# show details
print(ring_data.head())
row_num, col_num = ring_data.shape
print(row_num, col_num)
print(ring_data.describe())

# 特征的标准化
data = ring_data.iloc[:, 0:col_num - 1]
target = ring_data.iloc[:, col_num - 1]
...
def SVM(tr_x, tr_y, te_x, te_y):
    ...
    svm_model = svm.SVC(C=0.1, probability=True)
    svm_model.fit(tr_x, tr_y)
...
def tree_fit(estimators, tr_x, tr_y, te_x, te_y):
    # param_grid_depth = {'max_depth': np.arange(20, 30, 2)}
    ...
    tree_model = RandomForestClassifier(n_estimators=estimators, max_depth=21)
...
tree_fit(166, x_train, y_train, x_test, y_test)
