...
# input data
page_data = pd.read_csv('page-blocks.csv', sep=',')

# show details
print(page_data.head())
row_num, col_num = page_data.shape
print(row_num, col_num)
print(page_data.describe())

# 特征的标准化
data = page_data.iloc[:, 0:col_num - 1]
target = page_data.iloc[:, col_num - 1]
...
def SVM(tr_x, tr_y, te_x, te_y):
    ...
    print("TREE MSE=", caiculateMSE(te_y, y_pred_svm))
    one_hot = OneHotEncoder(sparse=False)
    y_true = one_hot.fit_transform(np.array(te_y).reshape(-1, 1))
...
def TREE(tr_x, tr_y, te_x, te_y):
    ...
    for i in range(60, 80, 1):
        ...
        score_lst.append(score)
    estimators = (score_lst.index(max(score_lst)) * 1) + 60
...
def tree_fit(estimators, tr_x, tr_y, te_x, te_y):
    ...
    tree_model = RandomForestClassifier(n_estimators=estimators, max_depth=13)
...
tree_fit(63, x_train, y_train, x_test, y_test)
