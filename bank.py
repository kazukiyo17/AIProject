...
row_num, col_num = bank_data.shape
...
missing = percent_missing(bank_data)
df_miss = sorted(missing.items(), key=lambda x: x[1], reverse=True)
print('Percent of missing data')
print(df_miss[0:10])

# 补全缺失值（众数）
bank_data['poutcome'] = bank_data['poutcome'].fillna('failure')
...
# 特征的标准化
data = bank_data.iloc[:, 0:col_num - 1]
target = bank_data.iloc[:, col_num - 1]
...
def tree_fit(estimators, tr_x, tr_y, te_x, te_y):
    # param_grid_depth = {'max_depth': np.arange(8, 13, 1)}
    ...
    tree_model = RandomForestClassifier(n_estimators=estimators, max_depth=14)
...
tree_fit(184, x_train, y_train, x_test, y_test)
