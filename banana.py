...
# input data
banana_data = pd.read_csv('banana.csv', sep=',')

# show details
print(banana_data.head())
row_num, col_num = banana_data.shape
print(row_num, col_num)
print(banana_data.describe())

# 特征的标准化
data = banana_data.iloc[:, 0:col_num - 1]
target = banana_data.iloc[:, col_num - 1]
...
def KNN(tr_x, tr_y, te_x, te_y):
    ...
    # neighbors = (score_lst.index(max(score_lst)) * 1) + 10
...
def TREE(tr_x, tr_y, te_x, te_y):
    ...
    for i in range(20, 40, 1):
        ...
        score_lst.append(score)
    estimators = (score_lst.index(max(score_lst)) * 1) + 20
...
def tree_fit(estimators, tr_x, tr_y, te_x, te_y):
    ...
    tree_model = RandomForestClassifier(n_estimators=estimators, max_depth=9)
...
tree_fit(35, x_train, y_train, x_test, y_test)
